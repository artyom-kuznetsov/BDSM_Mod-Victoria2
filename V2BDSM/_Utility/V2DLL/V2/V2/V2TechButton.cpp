...
static BytePatch EXE_PATCHES[] =
{
    ...
    // Кнопка "+" открывалась, но "Построить" после выбора фабрики -
    // нет: как и с колониями раньше, гейт цивилизованности задублирован
    // ещё в одном месте. FUN_0052e960 (та же функция, что и три патча
    // выше) для ветки "строим в своей провинции" (state.owner ==
    // наша страна) НЕ доходит до общей проверки +0x12D0 - вместо этого
    // сразу возвращает отдельный байт-флаг mov al,[EDI+0xAC8] со
    // страны. У нецивилизованных стран этот байт, судя по всему, 0.
    // Подменяем чтение на mov al,1 - ветка своей территории всегда
    // разрешена (остальные условия функции - лимит фабрик, колония -
    // остаются в силе, они проверяются раньше и этот патч не трогает).
    { "build_factory_ignore_uncivilized_own_territory", 0, 0x12E987, 6,
        { 0x8A, 0x87, 0xC8, 0x0A, 0x00, 0x00 },
        { 0xB0, 0x01, 0x90, 0x90, 0x90, 0x90 }, true },
