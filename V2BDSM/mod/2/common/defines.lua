...
country = {
	...
	NEWSPAPER_PRINTING_FREQUENCY = 350000, -- days frequency when the news attempt to be printed (may not print if not enough facts collected, and retry after another X days)
	...
	COLONIZATION_COLONY_STATE_DISTANCE = 250,
...
economy = {
	...
	MAX_LOAN_CAP_FROM_BANKS = 100, -- can loan max % of country tax base from single country
...
military = {
	...
	NAVAL_COMBAT_SEEKING_CHANCE = 1.0, -- base chance of picking a target (increased by leader reconaissance)
	NAVAL_COMBAT_SEEKING_CHANCE_MIN = 0.3, -- low cap for chance of picking the target
	NAVAL_COMBAT_SELF_DEFENCE_CHANCE = 5.0, -- scale up the chance of choosing the target that is already shooting at us.
	...
	NAVAL_COMBAT_DAMAGE_MULT_NO_ORG = 4.0, -- damage multiplier (to STR) when opponent has no ORG.
	NAVAL_COMBAT_RETREAT_CHANCE = 0.2, -- base chance for retreating when STR or ORG is low. Increased by leader experiance and speed.
	...
	NAVAL_COMBAT_RETREAT_SPEED_MOD = 0.5, -- slow down when retreating so there is a chance to hit the runner
	NAVAL_COMBAT_RETREAT_MIN_DISTANCE = 0.07, -- how close (in average) all alive ships must be to the center line to be able to retreat.
	...
	NAVAL_COMBAT_MAX_TARGETS = 1, -- max number of ships that may target the same enemy ship
...
diplomacy = {
	...
	TENSION_FROM_MOVEMENT_MAX = 0.02, -- max total value no matter the size of the movement
	AT_WAR_TENSION_DECAY = -0.24, -- for each potentially interested GP at war (value is if all possible GPs are at war) 
	TENSION_ON_CB_DISCOVERED = 0,
	TENSION_ON_REVOLT = 0,
	TENSION_WHILE_CRISIS = -1.0,
	CRISIS_COOLDOWN_MONTHS = 120,
	CRISIS_BASE_CHANCE = 0,
	CRISIS_TEMPERATURE_INCREASE = 0.0,
	...
	CRISIS_WARGOAL_PRESTIGE_MULT = 0, -- Applied to all prestige effects on wargoals in a crisis
	...
	LARGE_POPULATION_INFLUENCE_PENALTY = -0.3,
...
pops = {
	...
	PROMOTION_ASSIMILATION_CHANCE = 0.0,
	...
	LARGE_POPULATION_LIMIT = 15000000,
